...
describe('getImports', function() {
  it('returns default imports as dependencies', function() {
    ...
    expect(result).toEqual([expect.objectContaining({ label: 'foo', type: MemberType.dependency })]);
  });
  ...
  it('returns named imports as dependencies', function() {
    ...
    expect(result).toEqual([
      expect.objectContaining({ label: 'foo', type: MemberType.dependency }),
      expect.objectContaining({ label: 'bar', type: MemberType.dependency })
    ]);
  });
});
