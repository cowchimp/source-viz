...
export function getTopLevelFunctions(ast: TypescriptAst): TypescriptMember[] {
  ...
  const functions = topLevelStatements.filter<ts.FunctionDeclaration>(ts.isFunctionDeclaration);
  const publicFunctions = functions.filter(isPublic);
  const privateFunctions = difference(functions, publicFunctions);


  return [
    ...publicFunctions.map(x => new TypescriptMember(x, MemberType.publicMethod)),
    ...privateFunctions.map(x => new TypescriptMember(x, MemberType.privateMethod))
  ];
}
