...
export function ColumnFilters(props: ColumnFilterProps) {
  const counts = {
    [MemberType.dependency]: props.members.filter(x => x.type == MemberType.dependency).length,
    [MemberType.privateMethod]: props.members.filter(x => x.type == MemberType.privateMethod).length,
    [MemberType.publicMethod]: props.members.filter(x => x.type == MemberType.publicMethod).length
  };
