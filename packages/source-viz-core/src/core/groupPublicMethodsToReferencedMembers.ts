...
export function groupPublicMethodsToReferencedMembers(
  members: MemberInfo[],
  ...
  return members
    ...
    .reduce((acc, publicMethod) => {
      const linkedMembers = getMembersTransitively(
        publicMethod,
        transitiveLinkFilters,
        (m) => referencingMethodsToMembers.get(m) || [],
      );
...
function getMembersTransitively(
  root: MemberInfo,
  transitiveLinkFilters: Set<MemberType>,
  getReferencedMembers: (m: MemberInfo) => MemberInfo[],
): MemberInfo[] {
  const referencedInRoot = getReferencedMembers(root);

  const descendants = getDescendants(root, (m) => {
    return getReferencedMembers(m).filter((x) =>
      transitiveLinkFilters.has(x.type),
    );
  });
  const referencedInDescendants = descendants.reduce(
    (acc, x) => acc.concat(getReferencedMembers(x)),
    [],
  );

  return getUniques(referencedInRoot, referencedInDescendants);
}

function getReferencingMethodsToMembers(
  members: MemberInfo[],
): Map<MemberInfo, MemberInfo[]> {
