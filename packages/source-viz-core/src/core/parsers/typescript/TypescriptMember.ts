...
export class TypescriptMember {
  constructor(public node: ts.NamedDeclaration, public type: MemberType) {}
  ...
  public getMemberInfo(
    ...
    return {
      ...
      type: this.type,
      referencingMethods: getReferencingMethods(
        this.node,
        members,
        ast,
      ).map((x) => this.getLabel(x, ast)),
    };
  }

  private getLabel(node: ts.NamedDeclaration, ast: TypescriptAst): string {
    if (
      this.isAnonymousDefaultFunction(node) ||
      this.isAnonymousDefaultArrowFunction(node)
    ) {
      return 'default';
    }
