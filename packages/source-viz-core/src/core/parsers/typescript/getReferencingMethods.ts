...
const validReferencingMemberTypes = [
  MemberType.privateMethod,
  MemberType.publicMethod,
];

export function getReferencingMethods(
  ...
  return Array.from(
    references.reduce((acc, reference) => {
      const ancestors = getAncestors(reference, (node) => ast.getParent(node));
      if (ancestors.includes(node)) {
        return acc;
      }
      const referencingMethod = potentialReferencingMethods.find((x) =>
        ancestors.includes(x),
      );
