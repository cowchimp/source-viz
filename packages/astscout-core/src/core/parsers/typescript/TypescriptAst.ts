...
export class TypescriptAst {
  ...
  constructor(private code: string) {
    ...
    this._languageService = ts.createLanguageService(
      new InMemoryLanguageServiceHost({ [fileName]: code }),
    );
  ...
  getReferences(identifier: ts.Identifier): ts.Node[] {
    ...
    const referencedSymbols = this._languageService.findReferences(
      this._sourceFile.fileName,
      position,
    );
    const referenceEntries = referencedSymbols.reduce<ts.ReferenceEntry[]>(
      (acc, symbol) => acc.concat(symbol.references),
      [],
    );
  ...
  private visit(
    ...
    cb: (node: ts.Node, parent: ts.Node | undefined) => void,
  ) {
    cb(node, parent);
