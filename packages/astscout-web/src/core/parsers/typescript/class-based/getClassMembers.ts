...
export function getClassMembers(
  myClass: ts.ClassDeclaration,
  ast: TypescriptAst,
): TypescriptMember[] {
  const dependencies = getDependencies(myClass);
  const properMethods = myClass.members.filter<ts.MethodDeclaration>(
  ...
  const methods = (properMethods as ts.NamedDeclaration[]).concat(
    functionProperties,
  );
...
function getDependencies(myClass: ts.ClassDeclaration) {
  const constructor = myClass.members.find<ts.ConstructorDeclaration>(
  ...
  const privateConstructorParameters = filterByScope(
...
function filterByScope<T extends ts.Declaration>(
  nodes: ReadonlyArray<T>,
  scope: ts.ModifierFlags,
): T[] {
